...
#. handle_errors
read_ws_combine <- function(network, domain) {
    
    files <- list.files(glue('data/{n}/{d}/munged/', n = network, d = domain)) 
    ws_path <- grep('ws*', files, value = TRUE)
    
    ws_files <- list.files(path = glue('data/{n}/{d}/munged/{ws}', n = network, 
                                       d = domain, ws = ws_path), full.names = TRUE)
    
    for(i in 1:length(ws_files)) {
        sites <- st_read(ws_files[i])
        
        if(i == 1) {
            all_sites <- sites %>%
                filter(site_name == 'fake')
        }
        all_sites <- rbind(all_sites, sites)
    }
    return(sf::st_as_sf(all_sites))
}
...
get_gee_standard <- function(network, domain, var) {
    ...
    #commented line is if we wnat to remove very small watersheds that may have 
    #error due to the large size of pixles 
     fin <- full_join(median, sd) %>%
         full_join(count) #%>%
