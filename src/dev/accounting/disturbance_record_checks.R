...
sites_ = read_csv('~/Downloads/site_data - Sheet1(2).csv')
sites = filter(sites_, site_type != 'precip_gauge')
dist_ = read_csv('~/Downloads/disturbance_record - Sheet1(1).csv')
...
#status should be all-NA
sites_ %>%
    filter(site_type == 'rain_gauge') %>%
    select(domain, site_code, ws_status) %>%
    print(n=1000)

#status doesn't need to be all-NA, but maybe account for the rest? naw.
sites_ %>%
